...
/**
 * BattleUI — owns every Swing widget on the battle screen.
 * BattlePanel creates one instance, calls layoutAll() on resize,
 * and calls refreshUI(state) / refreshQuestion(state) each tick.
 *
 * Usage:
 *   BattleUI ui = new BattleUI(battlePanel, audioManager);
 *   ui.layoutAll();
 *   ui.refreshUI(state);
 */
